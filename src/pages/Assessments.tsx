...
export default function Assessments() {
  ...
  const jobsWithAssessments = jobs.map(job => ({
    ...job,
    assessment: assessments.find(a => a.jobId === job.id)
  }));
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            {user?.role === 'hr' 
              ...
              : 'Take assessments for job applications'
            }
          </p>
        </div>
        {user?.role === 'hr' && (
        ...
        {jobsWithAssessments.map((job) => (
          <Card key={job.id}>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <CardDescription className="mt-1">
                    {job.assessment ? (
                      `${job.assessment.sections.reduce((sum, s) => sum + s.questions.length, 0)} questions`
                    ) : (
                      'No assessment yet'
                    )}
                  </CardDescription>
            ...
            <CardContent>
              ...
              {/* Show questions preview for assessments */}
              ...
              {user?.role === 'hr' ? (
                ...
                <Button 
                  ...
                  onClick={() => {
                    if (job.assessment) {
                      // Navigate to assessment attempt page
                      navigate(`/assessments/${job.id}/attempt`);
                    }
                  }}
                >
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground mb-4">
              {user?.role === 'hr' ? 'No active jobs found' : 'No assessments available'}
            </p>
            {user?.role === 'hr' && (
