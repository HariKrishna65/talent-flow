...
import { db, Assessment, AssessmentSection, AssessmentQuestion, QuestionType, Job } from '@/lib/db';
...
export default function CreateAssignment() {
  ...
  // Generate job-related questions based on the selected job
  const generateJobRelatedQuestions = (job: Job): AssessmentQuestion[] => {
    const questions: AssessmentQuestion[] = [];
    
    // Question 1: Experience with job-specific technologies
    if (job.tags.length > 0) {
      questions.push({
        id: `q-${Date.now()}-1`,
        type: 'multi-choice',
        text: `Which of the following technologies from this ${job.title} role are you proficient in?`,
        required: true,
        options: job.tags
      });
    }

    // Question 2: Years of experience in the field
    questions.push({
      id: `q-${Date.now()}-2`,
      type: 'single-choice',
    ...
    // Question 3: Specific project experience
    questions.push({
      id: `q-${Date.now()}-3`,
      type: 'long-text',
      text: `Describe a challenging project you worked on that relates to ${job.title.toLowerCase()}. What was your role and what technologies did you use?`,
      required: true,
      maxLength: 1000
    });
    ...
    questions.push({
      id: `q-${Date.now()}-4`,
      type: 'long-text',
      text: `How would you approach solving a complex technical problem in a ${job.title.toLowerCase()} role? Walk us through your thought process.`,
      required: true,
      maxLength: 800
    });

    // Question 5: Motivation and interest
    questions.push({
      id: `q-${Date.now()}-5`,
      type: 'long-text',
      text: `What interests you most about this ${job.title} position? How do you see yourself contributing to our team?`,
      required: true,
      maxLength: 500
    });
  ...
  const handleJobSelection = (jobId: string) => {
    ...
    if (job) {
      ...
      setSections([{
        id: `section-${Date.now()}`,
        title: `${job.title} - Assessment Questions`,
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Choose the job position for which you want to create an assessment
          </CardDescription>
        </CardHeader>
        <CardContent>
                      ...
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Select
                              value={question.type}
                              onValueChange={(value) => updateQuestion(section.id, question.id, { type: value as QuestionType })}
                            >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <Input
                                ...
                                onChange={(e) => updateQuestion(section.id, question.id, { minValue: Number(e.target.value) })}
                                className="mt-2"
                              />
                            </div>
                            <div>
                              ...
                              <Input
                                ...
                                onChange={(e) => updateQuestion(section.id, question.id, { maxValue: Number(e.target.value) })}
                                className="mt-2"
                              />
                            </div>
                          </div>
                        ...
                        {(question.type === 'short-text' || question.type === 'long-text') && (
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateQuestion(section.id, question.id, { maxLength: Number(e.target.value) })}
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              if (value === 'none') {
                                updateQuestion(section.id, question.id, { conditionalOn: undefined });
                              } else {
                                updateQuestion(section.id, question.id, { 
                                  conditionalOn: { questionId: value, expectedValue: '' }
                                });
                              }
                            }}
                          >
                          ...
                          {question.conditionalOn && (
                            <Input
                              ...
                              onChange={(e) => updateQuestion(section.id, question.id, {
                                conditionalOn: { ...question.conditionalOn!, expectedValue: e.target.value }
                              })}
