...
function SortableCandidate({ candidate, jobTitle, onClick }: { candidate: Candidate; jobTitle?: string; onClick: () => void }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export default function CandidatesKanban() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // over.id could be either a stage ID or another candidate's ID
    let newStage: string;
    ...
    if (isStage) {
      ...
      // over.id is a candidate, find which stage they belong to
    ...
    // Optimistic update
    try {
      await db.candidates.update(candidateId, { stage: newStage as Candidate['stage'] });
      ...
      toast({
        title: "Stage Updated",
        description: `Moved ${candidate.name} to ${stageName}`,
      });
    } catch (error) {
      // Rollback on error
  ...
  const handleDragOver = (event: DragEndEvent) => {
    // Allow dropping over stage columns
  };
  ...
  const handleCreateCandidate = async () => {
    if (!newCandidate.name || !newCandidate.email || !newCandidate.jobId) {
      toast({
        ...
        description: "Name, email, and job are required",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${newCandidate.name} has been added to ${stages.find(s => s.id === newCandidate.stage)?.label}`,
      });
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new candidate and assign them to a stage
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setNewCandidate({ ...newCandidate, stage: value as Candidate['stage'] })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          {activeCand ? (
            <Card className="cursor-move shadow-lg">
              <CardContent className="p-3">
                ...
                {jobs.find(j => j.id === activeCand.jobId)?.title && (
                  <Badge variant="outline" className="mt-1 text-xs">
                    {jobs.find(j => j.id === activeCand.jobId)?.title}
                  </Badge>
                )}
