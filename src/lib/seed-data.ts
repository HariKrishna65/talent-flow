import { db, Job, Candidate, Assessment, AssessmentSection } from './db';
...
export async function seedDatabase() {
  ...
  // Seed assessments for all active jobs with 2-3 questions each
  ...
  for (let i = 0; i < activeJobs.length; i++) {
    ...
    // Generate job-specific questions based on the job title and tags
    const questions = [];
    ...
    if (jobTags.length > 0) {
      questions.push({
        ...
        required: true,
        options: jobTags.slice(0, 6) // Limit to 6 options
      });
    }
    ...
    // Question 3: Project experience or motivation (alternating)
    if (i % 2 === 0) {
      questions.push({
        id: `q-${i}-3`,
        type: 'long-text',
