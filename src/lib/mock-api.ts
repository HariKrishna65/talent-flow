...
async function simulateNetwork() {
  await delay(Math.random() * 1000 + 200); // 200-1200ms
...
export const handlers = [
  ...
  http.patch(`${BASE_URL}/jobs/:id`, async ({ params, request }) => {
    try {
      ...
      await db.jobs.update(params.id as string, updates);
  ...
  http.patch(`${BASE_URL}/jobs/:id/reorder`, async ({ params, request }) => {
    try {
      ...
      // 10% chance of failure for testing rollback
      if (Math.random() < 0.1) {
  ...
  http.patch(`${BASE_URL}/candidates/:id`, async ({ params, request }) => {
    try {
      ...
      // If stage is being updated, create timeline entry
      if (updates.stage) {
